...
from app.core.auth.dependencies import SuperAdminUser, get_current_user
...
async def run_worker_now(
    ...
    from app.db.models.worker_schedule import WorkerSchedule
    from app.workers.scheduler import trigger_task_now
...
async def retry_dlq_event(
    ...
    from app.workers.events.bus import retry_dlq_event as do_retry
...
async def get_ai_usage(
    ...
    by_day = [
        {"date": str(r.day.date()), "tokens": r.tokens, "executions": r.executions, "errors": r.errors}
    ...
    by_provider = {
        r.provider_used: {"executions": r.executions, "tokens": r.tokens, "errors": r.errors}
        for r in by_provider_result.all()
    }
    ...
    by_agent = {
        r.agent_name: {"executions": r.executions, "tokens": r.tokens, "errors": r.errors}
        for r in by_agent_result.all()
    }
