...
from app.db.repositories.scrape_job import ScrapeJobRepository
from app.core.services.scraper_client import listar_processos, detalhar_processo, baixar_documento
...
async def task_orquestrar_pipeline(
    ...
    async with AsyncSessionLocal() as session:
        ...
        job_repo = ScrapeJobRepository(session, tenant_id)
        ...
        await sync_repo.update(
            ...
            progresso_detalhado={
                "fase_atual": "listing",
                "tribunais": tribunais,
                "tribunais_status": {t: "pending" for t in tribunais},
                "total_processos": 0,
                "processados": 0,
                "total_docs": 0,
                "docs_baixados": 0,
...
async def _pipeline_tribunal(
    ...
    listing_result = await listar_processos(oab_numero, oab_uf, tribunal)
    ...
    if not listing_result.get("sucesso"):
        ...
        if listing_result.get("blocked"):
            ...
            return {"total": 0, "novos_processos": 0, "novas_movimentacoes": 0, "docs_baixados": 0}
    ...
    logger.info(f"pipeline_listed tribunal={tribunal} total={total} parsed={len(processos_basicos)}")
    ...
    for i, proc_basico in enumerate(processos_basicos):
        ...
        try:
            ...
            detail_result = await detalhar_processo(tribunal, numero, oab_numero, oab_uf)
